...
export async function GET() {
  const [{ data: bets }, { data: users }] = await Promise.all([
    supabase.from('friday_bets').select('user_id, player_name, tiebreaker, created_at').order('created_at'),
...
export async function POST(request) {
  ...
  if (!player || !tiebreaker) return new Response(JSON.stringify({ error: 'Mangler spiller eller tipset antall slag' }), { status: 400 });
  ...
  const { error } = await supabase.from('friday_bets').upsert(
